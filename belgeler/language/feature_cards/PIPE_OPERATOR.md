...
-- Lambda ile pipe
numeric result = 42
    |> (lambda(numeric x) as numeric x * 2 end_lambda)
    |> (lambda(numeric x) as numeric x + 1 end_lambda)
```
...
function main() as numeric
    ...
    if result == 11 then return 1 end_if
    return 0
end_function
```
...
**Kanonik Kaynak:** `pmlp1_sozdizimi.md` — Operatörler; `pmlp2_syntax.md` — Pipeline bölümü
