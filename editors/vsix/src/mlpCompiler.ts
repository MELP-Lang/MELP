...
export async function compileMlpProgram(options: CompileMlpProgramOptions): Promise<string> {
  ...
  try {
    ...
    // orkat_root: parent of dil/ (normalizer constructs diller_path from it)
    const orkatRoot = options.extensionPath
      ? options.extensionPath
      : normDillerPath.replace(/\/dil\/[a-z]+\/keywords\.json$/, '');
    fs.writeFileSync('/tmp/.melp_norm_src', fs.readFileSync(options.sourcePath));
    fs.writeFileSync('/tmp/.melp_norm_ext', langCode);         // normalizer reads _ext for lang detection
    fs.writeFileSync('/tmp/.melp_norm_ortak_root', orkatRoot); // normalizer constructs diller_path from this
    fs.writeFileSync('/tmp/.melp_norm_diller', normDillerPath); // legacy, kept for compat
    ...
    compileOutput += await runProcess(clang, ['-no-pie', objectPath, '-lm', '-o', options.outputPath], workDir);
