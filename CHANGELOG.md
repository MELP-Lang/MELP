...
## [0.2.0-alfa] - 2026-04-11

### Büyük Dönüm Noktası

**MELP artık kendini derleyebiliyor — Selfhosting kanıtlandı.**

MELP derleyicisi `.mlp` diliyle yazılmış ve kendi kaynak kodunu derleyebilir
hale geldi. Bu, projenin kuruluşundan bu yana hedeflenen temel kilometre taşıdır.

### Teknik Detay

- **Gen2 = Gen3 differential test:** `.mlp` ile yazılmış derleyici (Gen2) kendini
  derleyerek Gen3 binary'sini üretiyor; kaynak yorum satırı dışında sıfır semantik
  fark gözlemlendi.
- **Derleme yama betiği kaldırıldı:** Bootstrap sürecinde kullanılan Python yama
  betiği (`fix_ir.py`) artık derleme zincirinde rol almıyor — sıfır aktif pass.
- **48/48 golden test:** Yama betiği olmadan da tüm testler geçiyor.
- **30+ compiler modülü:** Derleyicinin tamamı `.mlp` ile yazıldı.

### Eklenenler (STAGE2 derleyici modülleri)

- `compiler/lexer.mlp` — tam tokenizer
- `compiler/dispatch_words.mlp` → 8 alt modüle bölündü
- `compiler/control_parser.mlp` → 4 alt modüle bölündü
- 30+ `.mlp` derleyici modülü, tümü self-hosted
- 48 golden test, hepsi PASS

---

## [0.1.2-alfa] - 2026-04-05

### Düzeltmeler

- **STAGE0 v1.2.5 — Double-Free Bug Fix (776/776 PASS):** `codegen_callable.cpp`'de per-fonksiyon state reset eksikliği giderildi. `pendingHeapString` ve `heapStringVars` artık her fonksiyon derlemesi öncesinde sıfırlanıyor. `string_interp/test_05_combined` örneği artık `exit 134` yerine temiz çıkıyor.
- **dil/ v1.2.0 downstream sync:** `numeric?` / `?.` / `E-NULL-01` özellikleri GITHUB bağımlılık tablosuna yansıtıldı.
- **AR anahtar kelimeler (93):** `end for: نهاية_لكل`, `numeric?: عدد؟` eklendi.
