...
typedef struct Statement {
    ...
    void* control_flow_data;      // WhileStatement*, IfStatement*, ForStatement*
    struct Statement *body;       // Body statements (for while/if/for)
    struct Statement *else_body;  // Else body (for if-else)
...
void statement_free_recursive(Statement *stmt);

// Helper: Free statement recursively (including body and control flow data)
void statement_free_recursive(Statement *stmt) {
    if (!stmt) return;
    
    // Free control flow data using proper free functions
    if (stmt->control_flow_data) {
        switch (stmt->type) {
            case STMT_WHILE:
                while_statement_free((WhileStatement*)stmt->control_flow_data);
                break;
            // TODO: Add STMT_IF and STMT_FOR cases when enum is updated
            default:
                break;
        }
    }
    ...
    // Free body (recursive!)
    if (stmt->body) {
        statement_free_recursive(stmt->body);
    }
    
    // Free else body
    if (stmt->else_body) {
        statement_free_recursive(stmt->else_body);
    }
...
void add_statement(FunctionDef *func, StmtType type, const char *content) {
    ...
    // Initialize control flow fields
    stmt->control_flow_data = NULL;
    stmt->body = NULL;
    stmt->else_body = NULL;
...
void parse_function_body(Lexer *lexer, FunctionDef *func, Token *first_tok) {
    ...
    while (tok->type != TOKEN_EOF) {
        // Check for control flow end markers - MUST come first!
        if (tok->type == TOKEN_END) {
            ...
            if (next->type == TOKEN_FUNCTION ||
                ...
                next->type == TOKEN_IF ||
                next->type == TOKEN_FOR) {
                // End of function or control flow block
                token_free(tok);
                token_free(next);
                return;  // Stop parsing!
            }
            // False alarm, continue parsing
            token_free(next);
        }
        ...
        if (tok->type == TOKEN_WHILE) {
            // Create control flow parser and parse condition
            ControlFlowParser* cfp = control_flow_parser_create(lexer);
            WhileStatement* while_data = control_flow_parse_while(cfp);
            
            // Token sync - critical!
            token_free(tok);
            tok = cfp->current_token;
            cfp->current_token = NULL;
            control_flow_parser_free(cfp);
            
            // Create statement manually (can't use add_statement)
            Statement *stmt = malloc(sizeof(Statement));
            stmt->type = STMT_WHILE;
            strcpy(stmt->content, "while");
            stmt->control_flow_data = while_data;
            stmt->body = NULL;
            stmt->else_body = NULL;
            stmt->next = NULL;
            
            // Recursive body parsing
            FunctionDef temp_func = {0};
            temp_func.locals = func->locals;
            temp_func.params = func->params;
            temp_func.local_count = func->local_count;
            temp_func.param_count = func->param_count;
            
            parse_function_body(lexer, &temp_func, NULL);
            
            // Get parsed body and update counts
            stmt->body = temp_func.body;
            func->local_count = temp_func.local_count;
                    ...
                    while (val_tok->type != TOKEN_EOF) {
                        ...
                        if (paren_depth == 0) {
                            if (val_tok->type == TOKEN_PRINT || val_tok->type == TOKEN_RETURN ||
                                ...
                                val_tok->type == TOKEN_NUMERIC || val_tok->type == TOKEN_STRING_TYPE ||
    ...
    while (tok->type != TOKEN_EOF) {
        ...
        if (tok->type == TOKEN_FUNCTION) {
            ...
            if (return_type_tok->type == TOKEN_NUMERIC || 
                return_type_tok->type == TOKEN_STRING_TYPE || 
...
// Forward declaration for recursive statement code generation
static void generate_statement_code(Statement* stmt, FILE* out, FunctionDef* func);
...
static void generate_statement_code(Statement* stmt, FILE* out, FunctionDef* func) {
    ...
    switch (stmt->type) {
        ...
        case STMT_WHILE: {
            // Get while statement data
            WhileStatement* while_data = (WhileStatement*)stmt->control_flow_data;
            if (!while_data || !while_data->condition) {
                break;
            }
            
            ComparisonExpr* cond = (ComparisonExpr*)while_data->condition;
            ...
            // Generate condition code using comparison module
            comparison_generate_code(out, cond);
            
            // Compare result with 0 (false)
            fprintf(out, "  cmp rax, 0\n");
            fprintf(out, "  je .while_end_%d      ; exit if condition false\n", label_num);
            
            // Generate body statements recursively
            Statement* body_stmt = stmt->body;
            while (body_stmt) {
                generate_statement_code(body_stmt, out, func);
...
void functions_codegen(FILE *out) {
    ...
    while (func != NULL) {
        ...
        Statement *stmt = func->body;
        while (stmt) {
            generate_statement_code(stmt, out, func);
            stmt = stmt->next;
        }
