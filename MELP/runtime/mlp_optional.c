...
MelpOptional* melp_optional_some(void* value, size_t value_size) {
    ...
    if (!opt) {
        panic("Failed to allocate memory for optional");
    }
...
MelpOptional* melp_optional_numeric(double value) {
    ...
    if (!heap_value) {
        panic("Failed to allocate memory for optional numeric");
    }
    *heap_value = value;
...
MelpOptional* melp_optional_string(const char* value) {
    ...
    if (!heap_value) {
        panic("Failed to allocate memory for optional string");
    }
...
MelpOptional* melp_optional_boolean(bool value) {
    ...
    if (!heap_value) {
        panic("Failed to allocate memory for optional boolean");
    }
    *heap_value = value;
...
void* melp_optional_get(MelpOptional* opt) {
    if (!opt) {
        panic("optional_get() called on NULL pointer");
    }
    if (opt->state == MELP_OPT_NONE) {
        panic("optional_get() called on None value");
    }
    return opt->value;
}
...
void* melp_optional_assert(MelpOptional* opt, const char* message) {
    if (!opt || opt->state == MELP_OPT_NONE) {
        if (message) {
            ...
            panic("Null assertion failed: value is None");
        }
    }
    return opt->value;
}
