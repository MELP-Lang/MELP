...
# MLP-GCC, SELFHOSTING, TODO_SELFHOSTING, TODO_SELFHOSTING.md placeholder'larını koru
```
...
**Sorun:** Dosyalarda hala `SELFHOSTING` görünüyor
...
```bash
cd TODO_PROBLEMATIC/
find . -type f -name "*.md" -exec sed -i 's/SELFHOSTING/GERCEK_AD/g' {} \;
```
